/**
 * Custom hook for fetching projects for Labour Attendance feature
 * Mirrors Notes feature's `useNotesProjects` behavior/transform.
 *
...
import { getLabourAttendanceProjects } from '../api/labourAttendanceApi';
import { showError } from '../../../utils/toast';

export const useLabourAttendanceProjects = (workspaceId) => {
  ...
  const fetchProjects = useCallback(async (forceRefetch = false) => {
    ...
    try {
      ...
      const response = await getLabourAttendanceProjects(currentWorkspaceId);
      ...
      console.error('Error fetching projects for labour attendance:', err);
      const errorMessage =
...
export default useLabourAttendanceProjects;

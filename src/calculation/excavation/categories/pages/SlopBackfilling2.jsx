...
const SlopBackfilling2 = () => {
    ...
    const h1 = parseFloat(heightH1) || 0;
    const h2 = parseFloat(heightH2) || 0;
    const d = parseFloat(tripVolume) || 0;
    const P = parseFloat(price) || 0;

    // Formulas as per image
    // Material Volume = 0.5 * (h1 + h2) * B * L
    const materialVolume = 0.5 * (h1 + h2) * B * L;
    const numberOfTrips = d !== 0 ? materialVolume / d : 0;
    ...
    const calculationData = [
        { labelKey: 'excavation.slopBackfilling2.length', symbol: 'L', value: `${L} m` },
        { labelKey: 'excavation.slopBackfilling2.base', symbol: 'B', value: `${B} m` },
        { labelKey: 'excavation.slopBackfilling2.heightH1', symbol: 'H1', value: `${h1} m` },
        { labelKey: 'excavation.slopBackfilling2.heightH2', symbol: 'H2', value: `${h2} m` },
        { labelKey: 'excavation.slopBackfilling2.tripVolume', symbol: 'T', value: `${d} m` },
        { labelKey: 'excavation.slopBackfilling2.price', symbol: 'P', value: `${P} ${t('roofArea.common.currency')} Per MT` },
    ];

    const detailedOutputs = [
        {
            titleKey: 'excavation.slopBackfilling2.results.materialVolume',
            labelKey: 'excavation.slopBackfilling2.results.materialVolume',
            labelSuffix: ' =',
            formula: '0.5X(H1+H2)XBXL',
            value: `${materialVolume.toFixed(3)} m3`,
        },
        {
            titleKey: 'excavation.slopBackfilling2.results.numberOfTrips',
            labelKey: 'excavation.slopBackfilling2.results.numberOfTrips',
            labelSuffix: ' =',
            formula: '(0.5X(H1+H2)XBXL)/D',
            value: `${numberOfTrips.toFixed(3)} Nos`,
        },
        {
            titleKey: 'excavation.slopBackfilling2.results.totalCost',
            labelKey: 'excavation.slopBackfilling2.results.totalCost',
            labelSuffix: ' =',
            formula: '((0.5X(H1+H2)XBXL)/D)XP',
    ...
    return (
        <div className="min-h-screen max-w-7xl mx-auto pb-20">
            <div className="mb-6">
                <PageHeader
                    title={t('excavation.slopBackfilling2.title')}
                    showBackButton
                    onBack={() => navigate(-1)}
                >
            ...
            <div className="bg-[#F9F4EE] rounded-3xl p-4 sm:p-6">
                <div className="flex items-center gap-4 sm:gap-10 pb-6 border-b border-[#060C120A]">
                    ...
                    <div className="flex items-center justify-center">
                        <img src={slopBackfilling2Icon} alt="Slop Backfilling 2" className="object-contain" />
                    </div>
                ...
                <div className="space-y-4 pt-4">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <InputField
                            ...
                            placeholder={t('excavation.slopBackfilling2.length')}
                        />
                        <InputField
                            unit="m"
                            value={base}
                            onChange={(e) => setBase(e.target.value)}
                            placeholder={t('excavation.slopBackfilling2.base')}
                        />
                        <InputField
                            unit="m"
                            value={heightH1}
                            onChange={(e) => setHeightH1(e.target.value)}
                            placeholder={t('excavation.slopBackfilling2.heightH1')}
                        />
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <InputField
                            ...
                            placeholder={t('excavation.slopBackfilling2.heightH2')}
                        />
                        <InputField
                            unit="m"
                            value={tripVolume}
                            onChange={(e) => setTripVolume(e.target.value)}
                            placeholder={t('excavation.slopBackfilling2.tripVolume')}
                        />
                        <InputField
                            value={price}
                            onChange={(e) => setPrice(e.target.value)}
                            placeholder={t('excavation.slopBackfilling2.price')}
                            suffix="₹/MT"
                        />
                    </div>
                </div>
                ...
                <div className="mt-10 animate-fade-in pb-10">
                        ...
                        <table className="w-full text-left border-collapse min-w-[320px]">
                            ...
                            <tbody className="divide-y divide-[#060C120A]">
                                <tr className="hover:bg-[#F9F9F9] transition-colors">
                                    <td className="px-6 py-4 text-sm text-primary border-r border-[#060C120A]">{t('excavation.slopBackfilling2.results.materialVolume')}</td>
                                ...
                                <tr className="hover:bg-[#F9F9F9] transition-colors">
                                    <td className="px-6 py-4 text-sm text-primary border-r border-[#060C120A]">{t('excavation.slopBackfilling2.results.numberOfTrips')}</td>
                                ...
                                <tr className="hover:bg-[#F9F9F9] transition-colors">
                                    <td className="px-6 py-4 text-sm text-primary border-r border-[#060C120A]">{t('excavation.slopBackfilling2.results.totalCost')}</td>
                    ...
                    <div className="flex flex-col md:flex-row gap-4 mt-8 mb-6">
                        ...
                        <div className="bg-[#F9F4EE] rounded-2xl px-6 flex justify-between items-center w-full md:w-1/2 h-[58px]">
                            <span className='text-primary font-medium'>{t('excavation.slopBackfilling2.results.totalCost')}</span>
                        ...
                        <Button
                            variant="primary"
                            onClick={() => navigate('/calculation/excavation/slop-backfilling-2/detailed', {
                                state: {
                                    calculationData,
            ...
            <DownloadPDFModal
                ...
                defaultTitle={t('excavation.slopBackfilling2.detailedTitle')}
...
export default SlopBackfilling2;

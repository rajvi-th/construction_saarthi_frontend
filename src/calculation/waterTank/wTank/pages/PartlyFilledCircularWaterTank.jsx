...
import partlyFilledCircularTankIcon from '../../../../assets/icons/partlyFilledCircularTanks.svg';

const PartlyFilledCircularWaterTank = () => {
    ...
    const L_level = parseFloat(waterLevel) || 0;

    // Formulas
    // Total Capacity = π * R² * H * 1000
    // Water Quantity = π * R² * L * 1000
    // Filled % = (Water Quantity / Total Capacity) * 100

    // Using 3.14159 as per image reference for consistency
    const PI = 3.14159;
    const totalCapacity = PI * R * R * H * 1000;
    const waterQuantity = PI * R * R * L_level * 1000;
    ...
    const calculationData = [
        { labelKey: 'waterTank.partlyFilledCircular.radius', symbol: 'R', value: `${R} m` },
        { labelKey: 'waterTank.partlyFilledCircular.height', symbol: 'H', value: `${H} m` },
        { labelKey: 'waterTank.partlyFilledCircular.waterLevel', symbol: 'L', value: `${L_level} m` },
    ];

    const detailedOutputs = [
        {
            titleKey: 'waterTank.partlyFilledCircular.results.totalCapacity',
            labelKey: 'waterTank.partlyFilledCircular.results.totalCapacity',
            labelSuffix: '',
            formula: '3.14159XRXRXHX1000',
            value: `${totalCapacity.toFixed(3)} Litre`,
        },
        {
            titleKey: 'waterTank.partlyFilledCircular.results.waterQuantity',
            labelKey: 'waterTank.partlyFilledCircular.results.waterQuantity',
            labelSuffix: ' =',
            formula: '3.14159XRXRXLX1000',
            value: `${waterQuantity.toFixed(3)} Litre`,
        },
        {
            titleKey: 'waterTank.partlyFilledCircular.results.filledPercentage',
            labelKey: 'waterTank.partlyFilledCircular.results.filledPercentage',
            labelSuffix: ' =',
            formula: '((3.14159XRXRXLX1000)/(3.14159XRXRXHX1000))X100',
            value: `${filledPercentage.toFixed(3)}`, // Image shows just value, but typically %
            unit: '%'
        },
    ];
    ...
    return (
        <div className="min-h-screen max-w-7xl mx-auto pb-20">
            <div className="mb-6">
                <PageHeader
                    title={t('waterTank.partlyFilledCircular.title')}
                    showBackButton
                    onBack={() => navigate(-1)}
                >
            ...
            <div className="bg-[#F9F4EE] rounded-3xl p-4 sm:p-6">
                <div className="flex items-center gap-4 sm:gap-10 pb-6 border-b border-[#060C120A]">
                    ...
                    <div className="flex items-center justify-center">
                        <img src={partlyFilledCircularTankIcon} alt="Partly Filled Circular Water Tank" className="object-contain" />
                    </div>
                    ...
                    <div className="space-y-4">
                        <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
                            <InputField
                                ...
                                placeholder={t('waterTank.partlyFilledCircular.radius')}
                            />
                            <InputField
                                unit="m"
                                value={height}
                                onChange={(e) => setHeight(e.target.value)}
                                placeholder={t('waterTank.partlyFilledCircular.height')}
                            />
                            <InputField
                                unit="m"
                                value={waterLevel}
                                onChange={(e) => setWaterLevel(e.target.value)}
                                placeholder={t('waterTank.partlyFilledCircular.waterLevel')}
                            />
                        </div>
                    </div>
                </div>
            ...
            {showResult && (
                <div className="mt-10 animate-fade-in pb-10">
                        ...
                        <table className="w-full text-left border-collapse min-w-[320px]">
                            ...
                            <tbody className="divide-y divide-[#060C120A]">
                                <tr className="hover:bg-[#F9F9F9] transition-colors">
                                    <td className="px-6 py-4 text-sm text-primary border-r border-[#060C120A]">{t('waterTank.partlyFilledCircular.results.totalCapacity')}</td>
                                ...
                                <tr className="hover:bg-[#F9F9F9] transition-colors">
                                    <td className="px-6 py-4 text-sm text-primary border-r border-[#060C120A]">{t('waterTank.partlyFilledCircular.results.waterQuantity')}</td>
                                    <td className="px-6 py-4 text-sm text-primary border-r border-[#060C120A]">{waterQuantity.toFixed(3)}</td>
                                    <td className="px-6 py-4 text-sm text-primary">Litre</td>
                                </tr>
                                <tr className="hover:bg-[#F9F9F9] transition-colors">
                                    <td className="px-6 py-4 text-sm text-primary border-r border-[#060C120A]">{t('waterTank.partlyFilledCircular.results.filledPercentage')}</td>
                    ...
                    <div className="flex justify-end">
                        <Button
                            variant="primary"
                            onClick={() => navigate(ROUTES_FLAT.CALCULATION_WATER_TANK_PARTLY_FILLED_CIRCULAR + '/detailed', {
                                state: {
                                    calculationData,
            ...
            <DownloadPDFModal
                ...
                defaultTitle={t('waterTank.partlyFilledCircular.detailedTitle')}
...
export default PartlyFilledCircularWaterTank;

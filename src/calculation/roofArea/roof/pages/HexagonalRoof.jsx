...
// Import icons
import hexagonalRoofs from '../../../../assets/icons/hexagonalRoofs.svg';
...
const HexagonalRoof = () => {
    ...
    const S = parseFloat(slopS) || 0;
    const P = parseFloat(roomPrice) || 0;

    const A_m = unitType === 'metric' ? A / 1000 : A;
    const S_m = unitType === 'metric' ? S / 1000 : S;

    // Roof Area Calculation from image: 3 * A * S
    const roofArea = 3 * A_m * S_m;
    ...
    const calculationData = [
        { labelKey: 'roofArea.hexagonalRoof.baseLengthA', symbol: 'A', value: `${(A / 1000).toFixed(3)} m` },
        { labelKey: 'roofArea.hexagonalRoof.slopS', symbol: 'S', value: `${(S / 1000).toFixed(3)} m` },
        { labelKey: 'roofArea.hexagonalRoof.roomPrice', symbol: 'P', value: roomPrice, unitKey: 'roofArea.common.currencyPerSqM' },
    ];

    const detailedOutputs = [
        {
            titleKey: 'roofArea.hexagonalRoof.results.roofArea',
            labelKey: 'roofArea.hexagonalRoof.results.roofArea',
            labelSuffix: ' =',
            formula: '3XaXs',
        ...
        {
            titleKey: 'roofArea.hexagonalRoof.results.roofCost',
            labelKey: 'roofArea.hexagonalRoof.results.roofCost',
            labelSuffix: ' =',
            formula: '(3XaXs)Xp',
    ...
    return (
        <div className="min-h-screen max-w-7xl mx-auto pb-20">
            <div className="mb-6">
                <PageHeader
                    title={t('roofArea.hexagonalRoof.title')}
                    showBackButton
                    onBack={() => navigate(-1)}
                >
            ...
            <div className="bg-[#F9F4EE] rounded-3xl p-4 sm:p-6">
                <div className="flex items-center gap-4 sm:gap-10 pb-6 border-b border-[#060C120A]">
                    ...
                    <div className="flex items-center justify-center">
                        <img src={hexagonalRoofs} alt="Hexagonal Roof Diagram" className=" object-contain" />
                    </div>
                    ...
                    <div className="space-y-4">
                        <div>
                            <InputField
                                ...
                                placeholder={t('roofArea.hexagonalRoof.baseLengthA')}
                            />
                        </div>
                        <div>
                            <InputField
                                unit="mm"
                                value={slopS}
                                onChange={(e) => setSlopS(e.target.value)}
                                placeholder={t('roofArea.hexagonalRoof.slopS')}
                            />
                        </div>
                        <div>
                            <InputField
                                ...
                                placeholder={t('roofArea.hexagonalRoof.roomPrice')}
                            />
                        </div>
                    </div>
                </div>
            ...
            {showResult && (
                <div className="mt-10 animate-fade-in pb-10">
                    ...
                    <div className="bg-white rounded-xl border border-[#060C121A] overflow-hidden mb-8 overflow-x-auto">
                        <table className="w-full text-left border-collapse min-w-[320px]">
                            ...
                            <tbody className="divide-y divide-[#060C120A]">
                                {[
                                    { materialKey: 'roofArea.hexagonalRoof.results.roofArea', quantity: roofArea.toFixed(3), unit: 'sq.m.' },
                                    { materialKey: 'roofArea.hexagonalRoof.results.roofCost', quantity: roofCost.toFixed(3), unit: '₹' },
                                ].map((row, index) => (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <Button
                            variant="primary"
                            onClick={() => navigate(ROUTES_FLAT.CALCULATION_ROOF_AREA_HEXAGONAL_ROOF_DETAILED, {
                                state: {
                                    calculationData,
            ...
            <DownloadPDFModal
                ...
                defaultTitle={t('roofArea.hexagonalRoof.detailedTitle')}

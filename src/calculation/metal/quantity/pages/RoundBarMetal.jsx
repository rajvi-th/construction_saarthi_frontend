...
import roundBarMetalDiagram from '../../../../assets/icons/roundBarMetals.svg';
...
const RoundBarMetal = () => {
    ...
    const L_val = parseFloat(length) || 0;
    const N_val = parseFloat(noOfUnits) || 0;
    const R_val = parseFloat(rateOfMetal) || 0;
    const dens_val = parseFloat(density) || 0;

    // Volume = (π * d² / 4) * L
    const area_mm2 = (Math.PI / 4) * Math.pow(D_val, 2);
    const volume_m3_per_unit = (area_mm2 * L_val) / 1000000000;
    ...
    return (
        <div className="min-h-screen max-w-7xl mx-auto pb-20">
            <div className="mb-6">
                <PageHeader
                    title={t('metal.quantity.roundBar')}
                    showBackButton
                    onBack={() => navigate(-1)}
                >
            ...
            {/* Main Content Card */}
            <div className="bg-[#FAF5F0] rounded-3xl p-4 sm:p-6">
                <div className="flex items-center gap-4 sm:gap-10 pb-6 border-b border-[#060C120A]">
                    {/* Diagram Box */}
                    <div className="flex items-center justify-center">
                        <img src={roundBarMetalDiagram} alt="Round Bar Diagram" className="w-full h-full object-contain max-h-48" />
                    </div>
                ...
                <div className="md:space-y-4 space-y-2 pt-6">
                    {/* Row 1: Side - d, Length - L */}
                    {/* Requirement: In small screen show in one row */}
                    <div className="grid grid-cols-2 gap-2 md:gap-4">
                        <InputField
                            placeholder={t('metal.roundBar.d')}
                            suffix="mm"
                            value={d}
                            onChange={(e) => setD(e.target.value)}
                        />
                        <InputField
                            placeholder={t('metal.roundBar.length')}
                            suffix="mm"
                            value={length}
            ...
            {showResult && (
                <div className="mt-10 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10">
                    ...
                    <div className="bg-white rounded-xl border border-[#060C121A] overflow-hidden overflow-x-auto shadow-sm">

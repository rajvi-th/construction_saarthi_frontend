...
import triangularPipeMetalDiagram from '../../../../assets/icons/triangularPipeBarMetals.svg';

const TriangularPipeMetal = () => {
    ...
    const T = parseFloat(thicknessT) || 0;
    ...
    // Area approx for Triangular Pipe: Perimeter * Thickness
    // Exact: Area = Area(Outer) - Area(Inner)
    // Inner side a' = a - 2*sqrt(3)*T
    const aInner = Math.max(0, a - (2 * Math.sqrt(3) * T));
    const area_mm2 = (Math.sqrt(3) / 4) * (Math.pow(a, 2) - Math.pow(aInner, 2));

    const volume_m3_per_unit = (area_mm2 * (L * 1000)) / 1000000000;
    ...
    return (
        <div className="min-h-screen max-w-7xl mx-auto pb-20">
            <div className="mb-6">
                <PageHeader
                    title={t('metal.quantity.triangularPipe')}
                    showBackButton
                    onBack={() => navigate(-1)}
                >
            ...
            <div className="bg-[#FAF5F0] rounded-3xl p-3 md:p-6">
                <div className="flex items-center gap-4 sm:gap-10 pb-6 border-b border-[#060C120A]">
                    ...
                    <div className="flex items-center justify-center">
                        <img src={triangularPipeMetalDiagram} alt="Triangular Pipe Diagram" className="w-full h-full object-contain max-h-48" />
                    </div>
                ...
                <div className="md:space-y-4 space-y-2 pt-6">
                    {/* Row 1: Side - a, Thickness - T, Length - L */}
                    {/* Requirement: In small screen show in one row */}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-2 md:gap-4">
                        <InputField
                            placeholder={t('metal.triangularPipe.sideA')}
                            suffix="mm"
                            value={sideA}
                            onChange={(e) => setSideA(e.target.value)}
                        />
                        <InputField
                            placeholder={t('metal.triangularPipe.thicknessT')}
                            suffix="mm"
                            value={thicknessT}
                            onChange={(e) => setThicknessT(e.target.value)}
                        />
                        <InputField
                            placeholder={t('metal.triangularPipe.lengthL')}
                            suffix="m"
                            value={lengthL}
...
export default TriangularPipeMetal;

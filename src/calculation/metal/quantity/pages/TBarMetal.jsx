...
import tBarMetalDiagram from '../../../../assets/icons/tBarMetals.svg';
...
const TBarMetal = () => {
    ...
    // Area = (a * T) + ((b - T) * T) in mm²
    const area_mm2 = (a * T) + ((b - T) * T);
    const volume_m3_per_unit = (area_mm2 * L) / 1000000; // area is mm2, weight is per m (L is in m usually for beams?)
    // Let's assume L is in m as per previous page modification (SquareTubing L was m)
    const unitWeight = (area_mm2 / 1000000) * L * dens_val;
            ...
            <div className="bg-[#FAF5F0] rounded-3xl p-3 md:p-6">
                <div className="flex items-center gap-4 sm:gap-10 pb-6 border-b border-[#060C120A]">
                    ...
                    <div className="flex items-center justify-center">
                        <img src={tBarMetalDiagram} alt="T-Bar Diagram" className="w-full h-full object-contain max-h-48" />
                    </div>
                ...
                <div className="md:space-y-4 space-y-2 pt-6">
                    ...
                    <div className="grid grid-cols-2 gap-2 md:gap-4">
                        <InputField
                            placeholder={t('metal.tBar.sideA')}
                            suffix="mm"
                            value={sideA}
                            onChange={(e) => setSideA(e.target.value)}
                        />
                        <InputField
                            placeholder={t('metal.tBar.sideB')}
                            suffix="mm"
                            value={sideB}
                    ...
                    {/* Row 2: Thickness - T, Length - L */}
                    <div className="grid grid-cols-2 gap-2 md:gap-4">
                        <InputField
                            placeholder={t('metal.tBar.thicknessT')}
                            suffix="mm"
                            value={thicknessT}
                            onChange={(e) => setThicknessT(e.target.value)}
                        />
                        <InputField
                            placeholder={t('metal.tBar.lengthL')}
                            suffix="m"
                            value={lengthL}
                    ...
                    {/* Standard Row 3: No of Units, Rate of Metal */}

...
// Import icons
import cWalls from '../../../../assets/icons/cWalls.svg';
...
const CWall = () => {
    ...
    // Wall Size
    const [wallL, setWallL] = useState(''); // Length (m)
    const [wallW1, setWallW1] = useState(''); // Width 1 (m)
    const [wallW2, setWallW2] = useState(''); // Width 2 (m)
    const [wallH, setWallH] = useState(''); // Height (m)
    const [wallT, setWallT] = useState(''); // Thickness (m)
    ...
    const w1_m_wall = parseFloat(wallW1) || 0;
    const w2_m_wall = parseFloat(wallW2) || 0;
    ...
    // Volume of Wall = ((l+w+x)XhXt) - Based on user image formula
    const V = (l_m_wall + w1_m_wall + w2_m_wall) * h_m_wall * t_m_wall;
    ...
    const handleReset = () => {
        ...
        setWallL(''); setWallW1(''); setWallW2(''); setWallH(''); setWallT('');
    ...
    const calculationData = [
        ...
        { labelKey: 'brickWorkAndPlaster.clayBrick.width1', symbol: 'w', value: wallW1 || 0, unit: 'm' },
        { labelKey: 'brickWorkAndPlaster.clayBrick.width2', symbol: 'x', value: wallW2 || 0, unit: 'm' },
    ...
    const detailedOutputs = [
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.volumeOfWall', labelKey: 'brickWorkAndPlaster.clayBrick.results.volumeOfWall', labelSuffix: ' =', formula: '((l+w+x)XhXt)', value: V.toFixed(3), unit: 'm³' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.noOfBricks', labelKey: 'brickWorkAndPlaster.clayBrick.results.noOfBricks', labelSuffix: ' =', formula: '((l+w+x)XhXt)/((L+0.1)XWX(T+0.1))', value: noOfBricks.toFixed(3), unit: 'NOS' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.mortarDryVolume', labelKey: 'brickWorkAndPlaster.clayBrick.results.mortarDryVolume', labelSuffix: ' =', formula: '((((l+w+x)XhXt)-(((l+w+x)XhXt)/((L+0.1)XWX(T+0.1)))XLXWXT))X1.42)', value: mortarDryVolume.toFixed(3), unit: 'm³' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.cementBags', labelKey: 'brickWorkAndPlaster.clayBrick.results.cementBags', labelSuffix: ' =', formula: '(cX((((l+w+x)XhXt)-(((l+w+x)XhXt)/((L+0.1)XWX(T+0.1)))XLXWXT))X1.42))/((c+s)X0.035)', value: cementBags.toFixed(3), unitKey: 'history.units.bags' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.sandVolume', labelKey: 'brickWorkAndPlaster.clayBrick.results.sandVolume', labelSuffix: ' =', formula: '(sX((((l+w+x)XhXt)-(((l+w+x)XhXt)/((L+0.1)XWX(T+0.1)))XLXWXT))X1.42))/(c+s)', value: sandVolume.toFixed(3), unit: 'm³' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.brickCost', labelKey: 'brickWorkAndPlaster.clayBrick.results.brickCost', labelSuffix: ' =', formula: 'B1X((l+w+x)XhXt)/((L+0.1)XWX(T+0.1))', value: brickCost.toFixed(3), unitKey: 'brickWorkAndPlaster.clayBrick.currency' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.cementCost', labelKey: 'brickWorkAndPlaster.clayBrick.results.cementCost', labelSuffix: ' =', formula: '(C1X(cX((((l+w+x)XhXt)-(((l+w+x)XhXt)/((L+0.1)XWX(T+0.1)))XLXWXT))X1.42))/((c+s)X0.035))', value: cementCost.toFixed(3), unitKey: 'brickWorkAndPlaster.clayBrick.currency' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.sandCost', labelKey: 'brickWorkAndPlaster.clayBrick.results.sandCost', labelSuffix: ' =', formula: '(S1X(sX((((l+w+x)XhXt)-(((l+w+x)XhXt)/((L+0.1)XWX(T+0.1)))XLXWXT))X1.42))/(c+s))', value: sandCost.toFixed(3), unitKey: 'brickWorkAndPlaster.clayBrick.currency' },
    ...
    return (
        <div className="min-h-screen max-w-7xl mx-auto pb-20 px-4">
            <div className="mb-6">
                <PageHeader
                    title={t('brickWorkAndPlaster.shapes.cWall')}
                    showBackButton
                    onBack={() => navigate(-1)}
                >
            ...
            <div className="bg-[#F9F4EE] rounded-3xl p-4 sm:p-6">
                <div className="flex items-center gap-4 sm:gap-10 pb-6 border-b border-[#060C120A]">
                    <div className="flex items-center justify-center">
                        <img src={cWalls} alt={t('brickWorkAndPlaster.shapes.cWall')} className="object-contain" />
                    </div>
                </div>
                ...
                <div className="space-y-4 pt-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-2 md:gap-4">
                            ...
                            <InputField unit="m" value={wallW1} onChange={(e) => setWallW1(e.target.value)} placeholder={t('brickWorkAndPlaster.clayBrick.width1')} />
                            <InputField unit="m" value={wallW2} onChange={(e) => setWallW2(e.target.value)} placeholder={t('brickWorkAndPlaster.clayBrick.width2')} />
                            <div className='md:col-span-3 flex gap-2 md:gap-4'>
            ...
            {showResult && (
                <div className="mt-10 animate-fade-in pb-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <Button
                            variant="primary"
                            onClick={() => navigate(ROUTES_FLAT.CALCULATION_SHAPES_BY_C_WALL_DETAILED, { state: { calculationData, outputs: detailedOutputs } })}
            ...
            <DownloadPDFModal
                ...
                defaultTitle={t('brickWorkAndPlaster.shapes.cWallDetailedTitle')}

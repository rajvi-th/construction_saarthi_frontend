...
// Import icons
import lWalls from '../../../../assets/icons/lWalls.svg';
...
const LWall = () => {
    ...
    const [wallW, setWallW] = useState(''); // m
    const [wallH, setWallH] = useState(''); // m
    const [wallT, setWallT] = useState(''); // m (Thickness)
    ...
    // Volume of Wall = ((l+w)XhXt) - Simplified as per user image formula
    const V = (l_m_wall + w_m_wall) * h_m_wall * t_m_wall;

    // No Of Bricks = Volume / ((L+j)XWX(T+j))
    // We use W_m without joint as standard for many brickwork calculations unless it's a specific bond
    ...
    const detailedOutputs = [
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.volumeOfWall', labelKey: 'brickWorkAndPlaster.clayBrick.results.volumeOfWall', labelSuffix: ' =', formula: '((l+w)XhXt)', value: V.toFixed(3), unit: 'm³' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.noOfBricks', labelKey: 'brickWorkAndPlaster.clayBrick.results.noOfBricks', labelSuffix: ' =', formula: '((l+w)XhXt)/((L+0.01)XWX(T+0.01))', value: noOfBricks.toFixed(3), unit: 'NOS' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.mortarDryVolume', labelKey: 'brickWorkAndPlaster.clayBrick.results.mortarDryVolume', labelSuffix: ' =', formula: '((((l+w)XhXt)-(((l+w)XhXt)/((L+0.01)XWX(T+0.01)))XLXWXT))X1.42)', value: mortarDryVolume.toFixed(3), unit: 'm³' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.cementBags', labelKey: 'brickWorkAndPlaster.clayBrick.results.cementBags', labelSuffix: ' =', formula: '(cX((((l+w)XhXt)-(((l+w)XhXt)/((L+0.01)XWX(T+0.01)))XLXWXT))X1.42))/((c+s)X0.035)', value: cementBags.toFixed(3), unitKey: 'history.units.bags' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.sandVolume', labelKey: 'brickWorkAndPlaster.clayBrick.results.sandVolume', labelSuffix: ' =', formula: '(sX((((l+w)XhXt)-(((l+w)XhXt)/((L+0.01)XWX(T+0.01)))XLXWXT))X1.42))/(c+s)', value: sandVolume.toFixed(3), unit: 'm³' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.brickCost', labelKey: 'brickWorkAndPlaster.clayBrick.results.brickCost', labelSuffix: ' =', formula: 'B1X((l+w)XhXt)/((L+0.01)XWX(T+0.01))', value: brickCost.toFixed(3), unitKey: 'brickWorkAndPlaster.clayBrick.currency' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.cementCost', labelKey: 'brickWorkAndPlaster.clayBrick.results.cementCost', labelSuffix: ' =', formula: '(C1X(cX((((l+w)XhXt)-(((l+w)XhXt)/((L+0.01)XWX(T+0.01)))XLXWXT))X1.42))/((c+s)X0.035))', value: cementCost.toFixed(3), unitKey: 'brickWorkAndPlaster.clayBrick.currency' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.sandCost', labelKey: 'brickWorkAndPlaster.clayBrick.results.sandCost', labelSuffix: ' =', formula: '(S1X(sX((((l+w)XhXt)-(((l+w)XhXt)/((L+0.01)XWX(T+0.01)))XLXWXT))X1.42))/(c+s))', value: sandCost.toFixed(3), unitKey: 'brickWorkAndPlaster.clayBrick.currency' },
    ...
    return (
        <div className="min-h-screen max-w-7xl mx-auto pb-20">
            <div className="mb-6">
                <PageHeader
                    title={t('brickWorkAndPlaster.shapes.lWall')}
                    showBackButton
                    onBack={() => navigate(-1)}
                >
            ...
            <div className="bg-[#F9F4EE] rounded-3xl p-4 sm:p-6">
                <div className="flex items-center gap-4 sm:gap-10 pb-6 border-b border-[#060C120A]">
                    <div className="flex items-center justify-center">
                        <img src={lWalls} alt={t('brickWorkAndPlaster.shapes.lWall')} className="object-contain" />
                    </div>
                </div>
            ...
            {showResult && (
                <div className="mt-10 animate-fade-in pb-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <Button
                            variant="primary"
                            onClick={() => navigate(ROUTES_FLAT.CALCULATION_SHAPES_BY_L_WALL_DETAILED, { state: { calculationData, outputs: detailedOutputs } })}
            ...
            <DownloadPDFModal
                ...
                defaultTitle={t('brickWorkAndPlaster.shapes.lWallDetailedTitle')}

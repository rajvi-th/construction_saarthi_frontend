...
// Import icons
import wallWithArcDoors from '../../../../assets/icons/wallWithArcDoors.svg';

const WallWithArcDoor = () => {
    ...
    const [wallW, setWallW] = useState(''); // w
    const [wallH, setWallH] = useState(''); // h
    const [wallT, setWallT] = useState(''); // t
    const [joint, setJoint] = useState('10'); // mm (default 10mm)

    // Door Size (m)
    const [doorL, setDoorL] = useState(''); // D
    const [doorH, setDoorH] = useState(''); // H
    ...
    const t_m_wall = parseFloat(wallT) || 0;

    const d_m_door = parseFloat(doorL) || 0;
    const h_m_door = parseFloat(doorH) || 0;
    ...
    // Volume of Wall = ((2*l+2*w)*h*t) - (D*H*t)
    const totalWallVol = ((2 * l_m_wall) + (2 * w_m_wall)) * h_m_wall * t_m_wall;
    const doorVol = d_m_door * h_m_door * t_m_wall;
    const V = totalWallVol - doorVol;

    // No Of Bricks = Volume / ((L+j)XWX(T+j))
    ...
    const handleReset = () => {
        ...
        setWallL(''); setWallW(''); setWallH(''); setWallT('');
        setDoorL(''); setDoorH('');
    ...
    const calculationData = [
        ...
        { labelKey: 'brickWorkAndPlaster.clayBrick.wallThickness', symbol: 't', value: wallT || 0, unit: 'm' },
        { labelKey: 'brickWorkAndPlaster.clayBrick.doorLength', symbol: 'D', value: doorL || 0, unit: 'm' },
        { labelKey: 'brickWorkAndPlaster.clayBrick.doorHeight', symbol: 'H', value: doorH || 0, unit: 'm' },
    ...
    const detailedOutputs = [
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.volumeOfWall', labelKey: 'brickWorkAndPlaster.clayBrick.results.volumeOfWall', labelSuffix: ' =', formula: '((2*l+2*w)XhXt-(D*H*t))', value: V.toFixed(3), unit: 'm³' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.noOfBricks', labelKey: 'brickWorkAndPlaster.clayBrick.results.noOfBricks', labelSuffix: ' =', formula: '((2*l+2*w)XhXt-(D*H*t))/((L+0.1)XWX(T+0.1))', value: noOfBricks.toFixed(3), unit: 'NOS' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.mortarDryVolume', labelKey: 'brickWorkAndPlaster.clayBrick.results.mortarDryVolume', labelSuffix: ' =', formula: '((((2*l+2*w)XhXt-(D*H*t))-(((2*l+2*w)XhXt-(D*H*t))/((L+0.1)XWX(T+0.1)))XLXWXT))X1.42)', value: mortarDryVolume.toFixed(3), unit: 'm³' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.cementBags', labelKey: 'brickWorkAndPlaster.clayBrick.results.cementBags', labelSuffix: ' =', formula: '(cX((((2*l+2*w)XhXt-(D*H*t))-(((2*l+2*w)XhXt-(D*H*t))/((L+0.1)XWX(T+0.1)))XLXWXT))X1.42))/((c+s)X0.035)', value: cementBags.toFixed(3), unitKey: 'history.units.bags' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.sandVolume', labelKey: 'brickWorkAndPlaster.clayBrick.results.sandVolume', labelSuffix: ' =', formula: '(sX((((2*l+2*w)XhXt-(D*H*t))-(((2*l+2*w)XhXt-(D*H*t))/((L+0.1)XWX(T+0.1)))XLXWXT))X1.42))/(c+s)', value: sandVolume.toFixed(3), unit: 'm³' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.brickCost', labelKey: 'brickWorkAndPlaster.clayBrick.results.brickCost', labelSuffix: ' =', formula: 'B1X((2*l+2*w)XhXt-(D*H*t))/((L+0.1)XWX(T+0.1))', value: brickCost.toFixed(3), unitKey: 'brickWorkAndPlaster.clayBrick.currency' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.cementCost', labelKey: 'brickWorkAndPlaster.clayBrick.results.cementCost', labelSuffix: ' =', formula: '(C1X(cX((((2*l+2*w)XhXt-(D*H*t))-(((2*l+2*w)XhXt-(D*H*t))/((L+0.1)XWX(T+0.1)))XLXWXT))X1.42))/((c+s)X0.035))', value: cementCost.toFixed(3), unitKey: 'brickWorkAndPlaster.clayBrick.currency' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.sandCost', labelKey: 'brickWorkAndPlaster.clayBrick.results.sandCost', labelSuffix: ' =', formula: '(S1X(sX((((2*l+2*w)XhXt-(D*H*t))-(((2*l+2*w)XhXt-(D*H*t))/((L+0.1)XWX(T+0.1)))XLXWXT))X1.42))/(c+s))', value: sandCost.toFixed(3), unitKey: 'brickWorkAndPlaster.clayBrick.currency' },
    ...
    return (
        <div className="min-h-screen max-w-7xl mx-auto pb-20">
            <div className="mb-6">
                <PageHeader
                    title={t('brickWorkAndPlaster.shapes.wallWithArcDoor')}
                    showBackButton
                    onBack={() => navigate(-1)}
                >
            ...
            <div className="bg-[#F9F4EE] rounded-3xl p-4 sm:p-6">
                <div className="flex items-center gap-4 sm:gap-10 pb-6 border-b border-[#060C120A]">
                    <div className="flex items-center justify-center">
                        <img src={wallWithArcDoors} alt="Wall With Arc Door" className="object-contain" />
                    </div>
                </div>
                ...
                <div className="space-y-4 pt-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-2 gap-2 md:gap-4">
                            ...
                            <InputField unit="m" value={wallT} onChange={(e) => setWallT(e.target.value)} placeholder={t('brickWorkAndPlaster.clayBrick.wallThickness')} />
                        </div>
                    </div>

                    {/* Door Size */}
                    <div className="space-y-2">
                        <h3 className="font-medium text-primary ml-1">{t('brickWorkAndPlaster.clayBrick.doorSize')}</h3>
                        <div className="grid grid-cols-2 gap-2 md:gap-4">
                            <InputField unit="m" value={doorL} onChange={(e) => setDoorL(e.target.value)} placeholder={t('brickWorkAndPlaster.clayBrick.doorLength')} />
                            <InputField unit="m" value={doorH} onChange={(e) => setDoorH(e.target.value)} placeholder={t('brickWorkAndPlaster.clayBrick.doorHeight')} />
                        </div>
                    </div>
            ...
            {showResult && (
                <div className="mt-10 animate-fade-in pb-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <Button
                            variant="primary"
                            onClick={() => navigate(ROUTES_FLAT.CALCULATION_SHAPES_BY_WALL_WITH_ARC_DOOR_DETAILED, { state: { calculationData, outputs: detailedOutputs } })}
            ...
            <DownloadPDFModal
                ...
                defaultTitle={t('brickWorkAndPlaster.shapes.wallWithArcDoorDetailedTitle')}
...
export default WallWithArcDoor;

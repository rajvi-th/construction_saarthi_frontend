...
// Import icons
import buttressWalls from '../../../../assets/icons/buttressWalls.svg';
...
const ButtressWall = () => {
    ...
    const [numButtress, setNumButtress] = useState(''); // N
    ...
    const N = parseFloat(numButtress) || 0;
    ...
    // Volume of Shape = N X (0.5 X w X h) (as per image formula string)
    const V = N * (0.5 * w_m_wall * h_m_wall);
    ...
    const handleReset = () => {
        ...
        setWallL(''); setWallW(''); setWallH(''); setNumButtress('');
        setCavityThickness('');
    ...
    const calculationData = [
        ...
        { labelKey: 'brickWorkAndPlaster.clayBrick.numberOfButtress', symbol: 'N', value: numButtress || 0, unit: '' },
    ...
    const detailedOutputs = [
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.volumeOfShape', labelKey: 'brickWorkAndPlaster.clayBrick.results.volumeOfShape', labelSuffix: ' =', formula: 'N X (0.5 X wxh)', value: V.toFixed(3), unit: 'm³' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.noOfBricks', labelKey: 'brickWorkAndPlaster.clayBrick.results.noOfBricks', labelSuffix: ' =', formula: '(NX (0.5 X w X h)) / ((L + 0.1) X W X (T + 0.1))', value: noOfBricks.toFixed(3), unit: 'NOS' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.mortarDryVolume', labelKey: 'brickWorkAndPlaster.clayBrick.results.mortarDryVolume', labelSuffix: ' =', formula: '((N X (0.5 X w X h)) - ((N X (0.5 X w X h))/((L + 0.1) X W X (T + 0.1)) X LX WXT)) X 1.42', value: mortarDryVolume.toFixed(3), unit: 'm³' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.cementBags', labelKey: 'brickWorkAndPlaster.clayBrick.results.cementBags', labelSuffix: ' =', formula: '(c X ((N X (0.5 X w X h)) - ((N X (0.5 X w X h)) / ((L + 0.1) X WX (T + 0.1)) X LXWXT)) X 1.42)) / ((c + s) X 0.035)', value: cementBags.toFixed(3), unitKey: 'history.units.bags' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.sandVolume', labelKey: 'brickWorkAndPlaster.clayBrick.results.sandVolume', labelSuffix: ' =', formula: '(s X ((N X (0.5 X w X h)) - ((NX (0.5 X w Xh)) / ((L + 0.1) X WX (T + 0.1)) X LXWXT)) X 1.42) / (c + s)', value: sandVolume.toFixed(3), unit: 'm³' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.volumeOfCavity', labelKey: 'brickWorkAndPlaster.clayBrick.results.volumeOfCavity', labelSuffix: ' =', formula: 'Tc X l X h', value: cavityVolume.toFixed(3), unit: 'm³' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.brickCost', labelKey: 'brickWorkAndPlaster.clayBrick.results.brickCost', labelSuffix: ' =', formula: '((N X (0.5 X w X h)) / ((L + 0.1) X W X (T + 0.1))) X B1', value: brickCost.toFixed(3), unitKey: 'brickWorkAndPlaster.clayBrick.currency' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.cementCost', labelKey: 'brickWorkAndPlaster.clayBrick.results.cementCost', labelSuffix: ' =', formula: '((c X ((N X (0.5 X w X h)) - ((NX (0.5 X w X h)) / ((L + 0.1) X W X (T + 0.1)) X L X WXT)) X 1.42)) / ((c + s) X 0.035)) X C1', value: cementCost.toFixed(3), unitKey: 'brickWorkAndPlaster.clayBrick.currency' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.sandCost', labelKey: 'brickWorkAndPlaster.clayBrick.results.sandCost', labelSuffix: ' =', formula: '((s X ((N X (0.5 X w X h)) - ((N X (0.5 X w X h)) / ((L + 0.1) X W X (T + 0.1)) X L X WXT)) X 1.42)) / (c + s)) X S1', value: sandCost.toFixed(3), unitKey: 'brickWorkAndPlaster.clayBrick.currency' },
    ...
    return (
        <div className="min-h-screen max-w-7xl mx-auto pb-20">
            <div className="mb-6">
                <PageHeader
                    title={t('brickWorkAndPlaster.shapes.buttressWall')}
                    showBackButton
                    onBack={() => navigate(-1)}
                >
            ...
            <div className="bg-[#F9F4EE] rounded-3xl p-4 sm:p-6">
                <div className="flex items-center gap-4 sm:gap-10 pb-6 border-b border-[#060C120A]">
                    <div className="flex items-center justify-center">
                        <img src={buttressWalls} alt="Buttress Wall" className="object-contain" />
                    </div>
                </div>
                ...
                <div className="space-y-4 pt-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-2 gap-2 md:gap-4">
                            ...
                            <InputField unit="m" value={numButtress} onChange={(e) => setNumButtress(e.target.value)} placeholder={t('brickWorkAndPlaster.clayBrick.numberOfButtress')} />
                        </div>
                    </div>
                    {/* Mortar Ratio */}
                    <div className="space-y-2">
            ...
            {showResult && (
                <div className="mt-10 animate-fade-in pb-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <Button
                            variant="primary"
                            onClick={() => navigate(ROUTES_FLAT.CALCULATION_SHAPES_BY_BUTTRESS_WALL_DETAILED, { state: { calculationData, outputs: detailedOutputs } })}
            ...
            <DownloadPDFModal
                ...
                defaultTitle={t('brickWorkAndPlaster.shapes.buttressWallDetailedTitle')}

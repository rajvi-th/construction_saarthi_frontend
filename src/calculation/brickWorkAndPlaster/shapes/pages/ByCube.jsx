...
// Import icons
import byCubes from '../../../../assets/icons/byCubes.svg';
...
const ByCube = () => {
    ...
    // No Of Bricks = (lXhXt)/((L+j)X(W+j)X(T+j))
    const brickWithJointVol = (L_m + j_m) * (W_m + j_m) * (T_m + j_m);
    ...
    const detailedOutputs = [
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.volumeOfShape', labelKey: 'brickWorkAndPlaster.clayBrick.results.volumeOfShape', labelSuffix: ' =', formula: '(lXhXt)', value: V.toFixed(3), unit: 'm3' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.noOfBricks', labelKey: 'brickWorkAndPlaster.clayBrick.results.noOfBricks', labelSuffix: ' =', formula: '((lXhXt)/((L+j)X(w+j)X(T+j)))', value: noOfBricks.toFixed(3), unit: 'NOS' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.mortarDryVolume', labelKey: 'brickWorkAndPlaster.clayBrick.results.mortarDryVolume', labelSuffix: ' =', formula: '(((lXhXt)-(((lXhXt)/((L+j)X(w+j)X(T+j)))XLXwXT))X1.42)', value: mortarDryVolume.toFixed(3), unit: 'm3' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.cementBags', labelKey: 'brickWorkAndPlaster.clayBrick.results.cementBags', labelSuffix: ' =', formula: '(cX(((lXhXt)-(((lXhXt)/((L+j)X(w+j)X(T+j)))XLXwXT))X1.42))/((c+s)X0.035)', value: cementBags.toFixed(3), unit: 'bags' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.sandVolume', labelKey: 'brickWorkAndPlaster.clayBrick.results.sandVolume', labelSuffix: ' =', formula: '(sX(((lXhXt)-(((lXhXt)/((L+j)X(w+j)X(T+j)))XLXwXT))X1.42))/(c+s)', value: sandVolume.toFixed(3), unit: 'm3' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.brickCost', labelKey: 'brickWorkAndPlaster.clayBrick.results.brickCost', labelSuffix: ' =', formula: '(B1X((lXhXt)/((L+j)X(w+j)X(T+j))))', value: brickCost.toFixed(3), unitKey: 'brickWorkAndPlaster.clayBrick.currency' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.cementCost', labelKey: 'brickWorkAndPlaster.clayBrick.results.cementCost', labelSuffix: ' =', formula: '(C1X((cX(((lXhXt)-(((lXhXt)/((L+j)X(w+j)X(T+j)))XLXwXT))X1.42))/((c+s)X0.035)))', value: cementCost.toFixed(3), unitKey: 'brickWorkAndPlaster.clayBrick.currency' },
        { titleKey: 'brickWorkAndPlaster.clayBrick.results.sandCost', labelKey: 'brickWorkAndPlaster.clayBrick.results.sandCost', labelSuffix: ' =', formula: '(S1X((sX(((lXhXt)-(((lXhXt)/((L+j)X(w+j)X(T+j)))XLXwXT))X1.42))/(c+s)))', value: sandCost.toFixed(3), unitKey: 'brickWorkAndPlaster.clayBrick.currency' },
    ...
    return (
        <div className="min-h-screen max-w-7xl mx-auto pb-20">
            <div className="mb-6">
                <PageHeader
                    title={t('brickWorkAndPlaster.shapes.byCube')}
                    showBackButton
                    onBack={() => navigate(-1)}
                >
            ...
            <div className="bg-[#F9F4EE] rounded-3xl p-4 sm:p-6">
                <div className="flex items-center gap-4 sm:gap-10 pb-6 border-b border-[#060C120A]">
                    <div className="flex items-center justify-center">
                        <img src={byCubes} alt={t('brickWorkAndPlaster.shapes.byCube')} className="object-contain" />
                    </div>
                </div>
            ...
            {showResult && (
                <div className="mt-10 animate-fade-in pb-10">
                    ...
                    <div className="bg-white rounded-xl border border-[#060C121A] overflow-hidden mb-8 overflow-x-auto">
                        <table className="w-full text-left border-collapse min-w-[320px]">
                            ...
                            <tbody className="divide-y divide-[#060C120A]">
                                {[
                                    { material: t('brickWorkAndPlaster.clayBrick.results.volumeOfShape'), quantity: V.toFixed(3), unit: 'm3' },
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <Button
                            variant="primary"
                            onClick={() => navigate(ROUTES_FLAT.CALCULATION_SHAPES_BY_CUBE_DETAILED, { state: { calculationData, outputs: detailedOutputs } })}
            ...
            <DownloadPDFModal
                ...
                defaultTitle={t('brickWorkAndPlaster.shapes.byCubeDetailedTitle')}

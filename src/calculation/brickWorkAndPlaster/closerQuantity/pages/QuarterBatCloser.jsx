...
// Import icons
import quarterBatClosers from '../../../../assets/icons/quarterBatClosers.svg';

const QuarterBatCloser = () => {
    ...
    // Volume Of Wall = l * h * W_m
    const volumeOfWall = l * h * W_m;

    // No of Quarter Bat Closer = (2 * (h / (T_m + J_m))) 
    const noOfQuarterBatCloser = (T_m + J_m) > 0 ? (2 * (h / (T_m + J_m))) : 0;
    ...
    const detailedOutputs = [
        ...
        {
            titleKey: 'brickWorkAndPlaster.clayBrick.results.noOfQuarterBatCloser',
            labelKey: 'brickWorkAndPlaster.clayBrick.results.noOfBricks',
            labelSuffix: ' =',
            formula: '(2 X (h / (T + j)))',
            value: `${noOfQuarterBatCloser.toFixed(3)} NOS`
        }
    ];
    ...
    return (
        <div className="min-h-screen max-w-7xl mx-auto pb-20">
            <div className="mb-6">
                <PageHeader
                    title={t('brickWorkAndPlaster.bonds.quarterBatCloserTitle')}
                    showBackButton
                    onBack={() => navigate(-1)}
                >
            ...
            <div className="bg-[#F9F4EE] rounded-3xl p-4 sm:p-6">
                <div className="flex items-center gap-4 sm:gap-10 pb-6 border-b border-[#060C120A]">
                    <div className="flex items-center justify-center">
                        <img src={quarterBatClosers} alt={t('brickWorkAndPlaster.bonds.quarterBatCloserTitle')} className="object-contain" />
                    </div>
                </div>
            ...
            {showResult && (
                <div className="mt-10 animate-fade-in pb-10">
                        ...
                        <table className="w-full text-left border-collapse min-w-[320px]">
                            ...
                            <tbody className="divide-y divide-[#060C120A]">
                                {[
                                    ...
                                    { material: t('brickWorkAndPlaster.clayBrick.results.noOfQuarterBatCloser'), quantity: noOfQuarterBatCloser.toFixed(3), unit: 'NOS' },
                                ].map((row, index) => (
                    ...
                    <div className="flex justify-end">
                        <Button variant="primary" onClick={() => navigate(ROUTES_FLAT.CALCULATION_QUARTER_BAT_CLOSER_DETAILED, { state: { calculationData, outputs: detailedOutputs } })} className="!rounded-2xl hover:bg-[#B02E0C] transition-all h-[50px] !px-8">{t('steel.weight.viewDetailed')}</Button>
                    </div>
                </div>
            )}
            ...
            <DownloadPDFModal
                ...
                defaultTitle={t('brickWorkAndPlaster.bonds.quarterBatCloserDetailedTitle')}
...
export default QuarterBatCloser;

...
import grecianSwimmingPoolIcon from '../../../../assets/icons/grecianSwimmingPools.svg';

const GrecianSwimmingPool = () => {
    ...
    const D_INPUT = parseFloat(d) || 0;
    const DEPTH = parseFloat(depth) || 0;

    // Grecian Area = (A * B) - (2 * C * D)
    const baseArea = (A * B) - (2 * C * D_INPUT);
    const volumeLitre = baseArea * DEPTH * 1000;
    const volumeGallon = baseArea * DEPTH * 264;
    ...
    const calculationData = [
        { labelKey: 'swimmingPool.grecian.lengthA', symbol: 'A', value: `${A} ${t('concrete.byVolume.mUnit')}` },
        { labelKey: 'swimmingPool.grecian.lengthB', symbol: 'B', value: `${B} ${t('concrete.byVolume.mUnit')}` },
        { labelKey: 'swimmingPool.grecian.c', symbol: 'C', value: `${C} ${t('concrete.byVolume.mUnit')}` },
        { labelKey: 'swimmingPool.grecian.d', symbol: 'D', value: `${D_INPUT} ${t('concrete.byVolume.mUnit')}` },
        { labelKey: 'swimmingPool.grecian.depth', symbol: 'd', value: `${DEPTH} ${t('concrete.byVolume.mUnit')}` },
    ];

    const detailedOutputs = [
        {
            titleKey: 'swimmingPool.grecian.results.volume',
            labelKey: 'swimmingPool.grecian.results.volume',
            labelSuffix: ' =',
            formula: '((A X B) - (2 X C X D)) X d X 1000',
        ...
        },
        {
            ...
            labelKey: 'swimmingPool.grecian.results.volume',
            labelSuffix: ' =',
            formula: '((A X B) - (2 X C X D)) X d X 264',
    ...
    return (
        <div className="min-h-screen max-w-7xl mx-auto pb-20">
            <div className="mb-6">
                <PageHeader
                    title={t('swimmingPool.grecian.title')}
                    showBackButton
                    onBack={() => navigate(-1)}
                >
            ...
            <div className="bg-[#F9F4EE] rounded-2xl p-4 sm:p-6">
                <div className="flex items-center gap-4 sm:gap-10 pb-6 border-b border-[#060C120A]">
                    ...
                    <div className="flex items-center">
                        <img src={grecianSwimmingPoolIcon} alt={t('swimmingPool.grecian.title')} className="w-full h-full object-contain" />
                    </div>
                ...
                <div className="space-y-4 pt-6">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <InputField
                            ...
                            placeholder={t('swimmingPool.grecian.lengthA')}
                        />
                        <InputField
                            unit={t('concrete.byVolume.mUnit')}
                            value={lengthB}
                            onChange={(e) => setLengthB(e.target.value)}
                            placeholder={t('swimmingPool.grecian.lengthB')}
                        />
                        <InputField
                            unit={t('concrete.byVolume.mUnit')}
                            value={c}
                            onChange={(e) => setC(e.target.value)}
                            placeholder={t('swimmingPool.grecian.c')}
                        />
                    </div>
                    <div className='grid md:grid-cols-2 gap-4'>
                        <InputField
                            ...
                            placeholder={t('swimmingPool.grecian.d')}
                        />
                        <InputField
                            unit={t('concrete.byVolume.mUnit')}
                            value={depth}
                            onChange={(e) => setDepth(e.target.value)}
                            placeholder={t('swimmingPool.grecian.depth')}
                        />
                    </div>
                </div>
            ...
            {showResult && (
                <div className="mt-10 animate-fade-in pb-10">
                        ...
                        <table className="w-full text-left border-collapse min-w-[320px]">
                            ...
                            <tbody className="divide-y divide-[#060C120A]">
                                <tr className="hover:bg-[#F9F9F9] transition-colors">
                                    <td className="px-6 py-4 text-sm text-primary border-r border-[#060C120A]">{t('swimmingPool.grecian.results.volume')}</td>
                    ...
                    <div className="flex justify-end">
                        <Button
                            variant="primary"
                            onClick={() => navigate('/calculation/swimming-pool/grecian/detailed', {
                                state: {
                                    calculationData,
            ...
            <DownloadPDFModal
                ...
                defaultTitle={t('swimmingPool.grecian.detailedTitle')}
...
export default GrecianSwimmingPool;

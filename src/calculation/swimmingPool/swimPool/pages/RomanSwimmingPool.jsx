...
import romanSwimmingPoolIcon from '../../../../assets/icons/romanSwimmingPools.svg';

const RomanSwimmingPool = () => {
    ...
    // Formula: (Rectangular Area + Circular Ends Area) * Depth
    // Total Area = (L * W) + (PI * R^2)
    const baseArea = (L * W) + (PI * Math.pow(R, 2));
    const volumeLitre = baseArea * D * 1000;
    const volumeGallon = baseArea * D * 264;
    ...
    const calculationData = [
        { labelKey: 'swimmingPool.roman.length', symbol: 'L', value: `${L} ${t('concrete.byVolume.mUnit')}` },
        { labelKey: 'swimmingPool.roman.width', symbol: 'W', value: `${W} ${t('concrete.byVolume.mUnit')}` },
        { labelKey: 'swimmingPool.roman.depth', symbol: 'd', value: `${D} ${t('concrete.byVolume.mUnit')}` },
    ];

    const detailedOutputs = [
        {
            titleKey: 'swimmingPool.roman.results.volume',
            labelKey: 'swimmingPool.roman.results.volume',
            labelSuffix: ' =',
            formula: '((L X W) + (π X r²)) X d X 1000',
    ...
    return (
        <div className="min-h-screen max-w-7xl mx-auto pb-20">
            <div className="mb-6">
                <PageHeader
                    title={t('swimmingPool.roman.title')}
                    showBackButton
                    onBack={() => navigate(-1)}
                >
            ...
            <div className="bg-[#F9F4EE] rounded-2xl p-4 sm:p-6">
                <div className="flex items-center gap-4 sm:gap-10 pb-6 border-b border-[#060C120A]">
                    ...
                    <div className="flex items-center">
                        <img src={romanSwimmingPoolIcon} alt={t('swimmingPool.roman.title')} className="w-full h-full object-contain" />
                    </div>
                ...
                <div className="space-y-4 pt-6">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <InputField
                            ...
                            placeholder={t('swimmingPool.roman.length')}
                        />
                        <InputField
                            unit={t('concrete.byVolume.mUnit')}
                            value={width}
                            onChange={(e) => setWidth(e.target.value)}
                            placeholder={t('swimmingPool.roman.width')}
                        />
                        <InputField
                            unit={t('concrete.byVolume.mUnit')}
                            value={depth}
                            onChange={(e) => setDepth(e.target.value)}
                            placeholder={t('swimmingPool.roman.depth')}
                        />
                    </div>
                </div>
            ...
            {showResult && (
                <div className="mt-10 animate-fade-in pb-10">
                        ...
                        <table className="w-full text-left border-collapse min-w-[320px]">
                            ...
                            <tbody className="divide-y divide-[#060C120A]">
                                <tr className="hover:bg-[#F9F9F9] transition-colors">
                                    <td className="px-6 py-4 text-sm text-primary border-r border-[#060C120A]">{t('swimmingPool.roman.results.volume')}</td>
                    ...
                    <div className="flex justify-end">
                        <Button
                            variant="primary"
                            onClick={() => navigate('/calculation/swimming-pool/roman/detailed', {
                                state: {
                                    calculationData,
            ...
            <DownloadPDFModal
                ...
                defaultTitle={t('swimmingPool.roman.detailedTitle')}
...
export default RomanSwimmingPool;

...
import rectangularSwimmingPoolIcon from '../../../../assets/icons/rectangularSwimmingPools.svg';

const RectangularSwimmingPool = () => {
    ...
    // Volume (Litre) = L * W * D * 1000
    // Volume (Gallon) = L * W * D * 264
    const volumeLitre = L * W * D * 1000;
    const volumeGallon = L * W * D * 264;
    ...
    const calculationData = [
        { labelKey: 'swimmingPool.rectangular.length', symbol: 'L', value: `${L} ${t('concrete.byVolume.mUnit')}` },
        { labelKey: 'swimmingPool.rectangular.width', symbol: 'W', value: `${W} ${t('concrete.byVolume.mUnit')}` },
        { labelKey: 'swimmingPool.rectangular.depth', symbol: 'd', value: `${D} ${t('concrete.byVolume.mUnit')}` },
    ];

    const detailedOutputs = [
        {
            titleKey: 'swimmingPool.rectangular.results.volume',
            labelKey: 'swimmingPool.rectangular.results.volume',
            labelSuffix: ' =',
            formula: 'L X W X d X 1000',
    ...
    return (
        <div className="min-h-screen max-w-7xl mx-auto pb-20">
            <div className="mb-6">
                <PageHeader
                    title={t('swimmingPool.rectangular.title')}
                    showBackButton
                    onBack={() => navigate(-1)}
                >
            ...
            <div className="bg-[#F9F4EE] rounded-2xl p-4 sm:p-6">
                <div className="flex items-center gap-4 sm:gap-10 pb-6 border-b border-[#060C120A]">
                    ...
                    <div className="flex items-center">
                        <img src={rectangularSwimmingPoolIcon} alt={t('swimmingPool.rectangular.title')} className="w-full h-full object-contain" />
                    </div>
                ...
                <div className="space-y-4 pt-6">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <InputField
                            ...
                            placeholder={t('swimmingPool.rectangular.length')}
                        />
                        <InputField
                            unit={t('concrete.byVolume.mUnit')}
                            value={width}
                            onChange={(e) => setWidth(e.target.value)}
                            placeholder={t('swimmingPool.rectangular.width')}
                        />
                        <InputField
                            unit={t('concrete.byVolume.mUnit')}
                            value={depth}
                            onChange={(e) => setDepth(e.target.value)}
                            placeholder={t('swimmingPool.rectangular.depth')}
                        />
                    </div>
                </div>
            ...
            {showResult && (
                <div className="mt-10 animate-fade-in pb-10">
                        ...
                        <table className="w-full text-left border-collapse min-w-[320px]">
                            ...
                            <tbody className="divide-y divide-[#060C120A]">
                                <tr className="hover:bg-[#F9F9F9] transition-colors">
                                    <td className="px-6 py-4 text-sm text-primary border-r border-[#060C120A]">{t('swimmingPool.rectangular.results.volume')}</td>
                    ...
                    <div className="flex justify-end">
                        <Button
                            variant="primary"
                            onClick={() => navigate('/calculation/swimming-pool/rectangular/detailed', {
                                state: {
                                    calculationData,
            ...
            <DownloadPDFModal
                ...
                defaultTitle={t('swimmingPool.rectangular.detailedTitle')}
...
export default RectangularSwimmingPool;

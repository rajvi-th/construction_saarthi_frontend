...
// Import icons
import lshBar from '../../../../assets/icons/lshBar.svg';
...
const LShapeBar = () => {
    ...
    // Calculation Logic
    // Inputs are typically: A (m), B (m), d (mm), N (nos)
    const A = parseFloat(lengthA) || 0;
    const B = parseFloat(lengthB) || 0;
    const d = parseFloat(diameter) || 0;
    const N = parseFloat(noOfBars) || 0;

    // Length of Each Bar = A + B - (2 * d)
    // Note: A and B are in meters. d is in mm. 
    // We need to convert d to meters for the length calculation: d_m = d / 1000
    // Formula in image: A+B-(2Xd)
    // We will assume the formula intends standard engineering units subtraction (keeping consistent units).
    const lengthOfEachBar = (A + B) - (2 * d / 1000);
    ...
    const detailedOutputs = [
        ...
        },
        {
            ...
            labelSuffix: '=',
            formula: '((((A+B-(2Xd))XN)XdXd)/162.28)',
            ...
            <div className="bg-[#F9F4EE] rounded-3xl p-4 sm:p-6">
                <div className="flex items-center gap-4 sm:gap-10 pb-6 border-b border-[#060C120A]">
                    ...
                    <div className="flex items-center justify-center">
                        <img src={lshBar} alt="L Shape Bar Diagram" className="w-full h-full object-contain" />
                    </div>
            ...
            {showResult && (
                <div className="mt-10 animate-fade-in pb-10">
                    ...
                    <div className="flex justify-end gap-6">
                        <Button
                            variant="primary"
                            onClick={() => navigate(ROUTES_FLAT.CALCULATION_L_SHAPE_BAR_DETAILED, {
                                state: {
                                    calculationData,

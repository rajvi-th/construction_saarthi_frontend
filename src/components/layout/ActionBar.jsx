...
export default function ActionBar({
  ...
  const finalSearchPlaceholder = searchPlaceholder || t('search', { defaultValue: 'Search' });
  const finalSortByLabel = sortByLabel || t('sortBy', { defaultValue: 'Sort By' });
  return (
    <div className={`flex flex-col md:flex-row gap-3 md:gap-4 ${className}`}>
      ...
      {showSort && sortBy && onSortChange && (
        ...
        >
          ...
          <span className="text-sm md:text-base">{finalSortByLabel}</span>
        </button>
      )}
